...
- (IBAction)estimateRoute:(id)sender {
    ...
    NSUInteger error = NMAPrefetchRequestErrorUnknown;
    [_prefetcher estimateMapDataSizeForRoute:_route radius:_radius error:&error];
...
- (IBAction)fetchRoute:(id)sender {
    ...
    NSUInteger error = NMAPrefetchRequestErrorUnknown;
    [_prefetcher fetchMapDataForRoute:_route radius:_radius error:&error];
...
- (void)prefetcher:(NMAMapDataPrefetcher *)prefetcher didEstimate:(NSInteger)dataSizeKB forRequestId:(NSInteger)requestId withSuccess:(BOOL)success
{
    if (success) {
...
- (void)prefetcher:(NMAMapDataPrefetcher *)prefetcher didUpdateProgress:(float)progress forRequestId:(NSInteger)requestId
{
    if (progress == 1.0) {
